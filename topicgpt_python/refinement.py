...
def merge_topics(
    ...
    topic_sent = topics_root.to_topic_list(desc=True, count=True)
...
def update_generation_file(
    ...
    with jsonlines.open(updated_file, mode='w') as writer:
...
def replace_topic_key(text, mapping, verbose=False):
    ...
    for key, value in mapping.items():
        if key != value and key in text:
            text = regex.sub(r'\b' + regex.escape(key) + r'\b', value, text)
            #if verbose:
...
def refine_topics(
    ...
    Parameters:
    - api (str): API to use ('openai', 'vertex', 'vllm', 'gemini', 'azure','aliyun').
