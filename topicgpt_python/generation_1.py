...
def generate_topics(
    ...
    # Adjusted regex pattern to handle Chinese characters and complex descriptions
    ...
    for i, doc in enumerate(tqdm(docs)):
        ...
        try:
            ...
            for t in topics:
                ...
                # Match and extract the topic level, name, and description
                groups = regex.match(topic_format, t)
                lvl, name, desc = int(groups[1]), groups[2].strip(), groups[3].strip()

                #print(lvl, name, desc)  # Debugging the extracted values
                ...
                if dups:  # Implement early stopping if no new topics are generated for a while
                    dups[0].count += 1
                    running_dups += 1
                    if running_dups > early_stop:
                        return responses, topics_list, topics_root
                else:
                    # Add the new topic to the root node
                    topics_root._add_node(lvl, name, 1, desc, topics_root.root)

                    # Update the topic list based on the updated topics_root
                    topics_list = topics_root.to_topic_list(desc=False, count=True)
...
def generate_topic_lvl1(
    ...
    print(f"Number of topics in topics_root: {len(topics_root.root.descendants)}")
    print(f"Number of topics in topics_list: {len(topics_list)}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--api",
        type=str,
        help="API to use ('openai', 'vertex', 'vllm', 'azure', 'gemini','aliyun')",
        default='aliyun',#"openai",
    )
